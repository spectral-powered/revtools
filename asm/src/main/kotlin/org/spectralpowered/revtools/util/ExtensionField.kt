...
class ExtensionField<R, T>(
    ...
    operator fun setValue(self: R, prop: KProperty<*>, value: T): T = value.apply {
        store[self] = this
        setterCallback?.let { self.it(this) }
    }
}
...
fun <T, R> fieldWithSetter(setterCallback: R.(T) -> Unit) = ExtensionField<R, T>(setterCallback = setterCallback)

fun <T, R> nullField(init: (R) -> T?) = NullableExtensionField(init)
fun <T, R> nullField() = NullableExtensionField<R, T?>()
fun <T, R> nullFieldWithSetter(setterCallback: R.(T?) -> Unit) = NullableExtensionField<R, T>(setterCallback = setterCallback)

fun <T, R> mutableSetField(init: (R) -> MutableSet<T> = { mutableSetOf() }) = ExtensionField(init)
fun <T, R, K> mutableMapField(init: (R) -> MutableMap<T, K> = { mutableMapOf() }) = ExtensionField(init)
fun <T, R> mutableListField(init: (R) -> MutableList<T> = { mutableListOf() }) = ExtensionField(init)

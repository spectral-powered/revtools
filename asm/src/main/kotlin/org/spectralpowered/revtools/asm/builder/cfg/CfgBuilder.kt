...
import org.spectralpowered.revtools.asm.InvalidOperandException
...
import org.spectralpowered.revtools.asm.analysis.IRVerifier
import org.spectralpowered.revtools.asm.analysis.NullTypeAdapter
import org.spectralpowered.revtools.asm.builder.cfg.impl.FrameStack
import org.spectralpowered.revtools.asm.builder.cfg.impl.FrameState
import org.spectralpowered.revtools.asm.builder.cfg.impl.LocalArray
...
import org.spectralpowered.revtools.asm.ir.value.AbstractUsageContext
...
import org.spectralpowered.revtools.asm.objectClass
import org.spectralpowered.revtools.asm.type.*
import org.spectralpowered.revtools.asm.util.print
...
class CfgBuilder(
    ...
    private fun convertMethodInsn(insn: MethodInsnNode, insnIndex: Int) {
        ...
        val klass = when {
            ...
            insn.owner.startsWith("[") -> group.objectClass
            else -> group[insn.owner]
        }
    ...
    private val AsmHandle.asHandle: Handle
        get() = Handle(this.tag, group[this.owner].getMethod(this.name, this.desc), this.isInterface)

    @Suppress("RecursivePropertyAccessor", "RecursivePropertyAccessor")
    private val AsmType.asAsmIrType: Any
        get() = when (this.sort) {
            ...
            org.objectweb.asm.Type.ARRAY -> types.getArrayType(this.elementType.asAsmIrType as Type)
            org.objectweb.asm.Type.OBJECT -> group[this.className.replace('.', '/')].asType
            org.objectweb.asm.Type.METHOD -> MethodDescriptor(
                this.argumentTypes.map { it.asAsmIrType as Type },
                this.returnType.asAsmIrType as Type
            )
...
//        }
    ...
    fun build(): MethodBody {
        ...
        RetvalBuilder(group, this).visitBody(body)
        CfgOptimizer(group, this).visitBody(body)
        NullTypeAdapter(group, this).visitBody(body)
        ThrowCatchNormalizer(group, this).visitBody(body)

        body.slotTracker.rerun()

        IRVerifier(group, this).visitBody(body)

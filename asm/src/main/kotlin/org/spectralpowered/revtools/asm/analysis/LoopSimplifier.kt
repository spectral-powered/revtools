...
import org.spectralpowered.revtools.asm.ir.value.MethodUsageContext
...
class LoopSimplifier(override val group: ClassGroup) : LoopVisitor {
    ...
    override fun visit(method: Method) {
        ...
        method.usageContext.also { ctx = it }.use {
            super.visit(method)
            IRVerifier(group, it).visit(method)
        }
    }
    ...
    override fun visitLoop(loop: Loop) {
        ...
        if (loop.allEntries.size != 1) {
            if (group.failOnError) throw KtException("Can't simplify loop with multiple entries")
            else return
        }
        buildPreHeader(loop)
        buildLatch(loop)
    ...
    private fun remapPhis(target: BasicBlock, from: Set<BasicBlock>, to: BasicBlock) = with(ctx) {
        for (phi in target.instructions.filterIsInstance<PhiInst>()) {
            ...
            val targetPhi = inst(group) { phi(phi.type, targetIncomings) }
    ...
    private fun buildPreHeader(loop: Loop) = with(ctx) {
        ...
        preHeader += inst(group) { goto(header) }
            ...
            if (newEntries != oldEntries) {
                ...
                when {
                    ...
                    oldDiff.isEmpty() -> {
                        ...
                        for (operand in bodyOperands) {
                            ...
                            val incomings = catch.allPredecessors.associateWith {
                                when (it) {
                                    ...
                                    in newDiff -> group.value.getZero(operand.type)
                                    ...
                                    new -> new.filterIsInstance<PhiInst>()
                                        ...
                                        ?: run {
                                            val newIncomings = new.predecessors.associateWith { pred ->
                                                if (pred in newDiff) group.value.getZero(operand.type)
                                                else operand
                                            }
                                            val newPhiOperand = inst(group) { phi(operand.type, newIncomings) }
                            ...
                            val newPhi = inst(group) { phi(operand.type, incomings) }
                        ...
                        for (phi in catch.filterIsInstance<PhiInst>()) {
                            val incomings = phi.incomings.toMutableMap()
                            val default = group.value.getZero(phi.type)

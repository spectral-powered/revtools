...
import org.spectralpowered.revtools.asm.builder.asm.ClassBuilder
import org.spectralpowered.revtools.asm.builder.cfg.LabelFilterer
...
class ClassReadError(msg: String) : AsmException(msg)
...
class AsmClassWriter(private val loader: ClassLoader, flags: Flags) : ClassWriter(flags.value) {
...
private fun ClassNode.toByteArray(
    ...
    this.inlineJsrs()
    val cw = AsmClassWriter(loader, flags)
    val adapter = when {
...
fun Class.toByteArray(
    ...
    flags: Flags = Flags.writeComputeAll,
    checkClass: Boolean = false
): ByteArray = ClassBuilder(group, this).build().toByteArray(loader, flags, checkClass)

fun Class.write(
    group: ClassGroup, loader: ClassLoader,
    path: Path = Paths.get("$fullName.class"),
    flags: Flags = Flags.writeComputeFrames,
    checkClass: Boolean = false
): File = `try` {
    ClassBuilder(group, this).build().write(loader, path, flags, checkClass)
}.also {
